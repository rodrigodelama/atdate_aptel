...
def get_current_time(target, mode, port, debug_trigger, new_param_x):
        ...
        try:
            if os.fork() == 0:
                ...
                try:
                    ...
                    client_x_value = struct.unpack("!i", client_packet)[0]
                    if debug_trigger == 1:
                        print ("value recieved form client:", int(client_x_value))
                    counter = 0
                    while True:
...
def main():
    ...
    try:
        ...
        elif (sys.argv.index("-x")):
            new_param_x = sys.argv[sys.argv.index("-x")+1]
            if debug_trigger == 1:
    ...
    if mode == UDP or mode == TCP:
        get_current_time(target, mode, port, debug_trigger, new_param_x)
    # Server
