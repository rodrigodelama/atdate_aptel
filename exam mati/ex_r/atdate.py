...
def get_current_time(target, mode, port, debug_trigger, new_param):
    ...
    if (mode == UDP):
        ...
        try:
            ...
            packet_n = struct.pack("!i", new_param)
            if debug_trigger == 1:
                print("Packet n: "+packet_n)
            try:
                client_socket.send(packet_n)
                while True:
                    try:
                        time_recieve(client_socket, debug_trigger)
                    except struct.error:
            ...
            if os.fork() == 0:
                ...
                try:
                    ...
                    while True:
                        ...
                        print("Attending, requests every", str(client_n_value), "seconds")
                        connection_socket.send(message)
                        sleep(client_n_value)
                except struct.error:
